...
gtf_file <- "/home/ignatius/PostDoc/2020/StaphCLASH2020/Data/Genomic/JKD6009/Annotation/Hyb_pyCRAC/GCF_900607245.1_JKD6009_genomic_merge_features_simple_annot_checked_edited.gtf"
rna_type_file <- "/home/ignatius/PostDoc/2020/StaphCLASH2020/Data/Genomic/JKD6009/Annotation/Hyb_pyCRAC/rna_type_priority.tab"
...
rna_coordinates <- gff_tbl %>%
  dplyr::mutate( seqnames = "Saa_6009_C1") %>%
...
# gff_tbl %>% 
#   dplyr::filter( ID == "Saa_6009_C1_srna1") 
# 
# gff_tbl %>%
#   distinct( type)
...
left_names_step_2 <- left_names_step_1 %>%
      ...
      dplyr::mutate(left_feature = str_split( feature_name, "\\|")  %>% purrr::map2_chr ( rna_type, 
...
right_names_step_2 <- right_names_step_1 %>%
      dplyr::inner_join( right_min_rank, by =c("rank" = "rank", 
                                               ...
                                               "rna_type" = "rna_type"))  %>%
      dplyr::select(-rank_row_id, -rank) %>%
      dplyr::mutate(right_feature = str_split( feature_name, "\\|")  %>% purrr::map2_chr ( rna_type, 
                                                                                          ~{ paste( c(.x[1:3], .y)  , collapse="|")}))
```    
