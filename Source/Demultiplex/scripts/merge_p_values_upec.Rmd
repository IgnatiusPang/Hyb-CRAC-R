...
# hyb_p_val_dir <- file.path( results_dir,  "E_coli_UPEC", "Hybrids_P_Values")
hyb_p_val_dir <- file.path( "/home/ignatius/ownCloud/Shared/E_coli_UPEC_Hfq/RNA-RNA_interaction_analysis_(hyb)/BC1_BC2_Processed_Data")

 
dataset_type <-    "UPEC"   #  "SM_Only" # "JT_Only" #

merged_hyb_p_val_dir <- file.path( results_dir,  "E_coli_UPEC",  "Merged_Hybrids_P_Values_Jul_2021", dataset_type)
...
# Cluster total left and right half distance
cluster_max_distance <- 600
```
...
list_of_files <- list(   file.path( hyb_p_val_dir,"Output_BC1", "Hyb_Annot", "all_NNNAGAGCN_BC1_L5Ba_Hfq_1.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC1", "Hyb_Annot", "all_NNNGTGAGCN_BC1_L5Bb_Hfq_2.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC1", "Hyb_Annot", "all_NNNCACTAGCN_BC1_L5Bc_Hfq_3.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNTAAGCN_BC2_L5Aa_CIP_1.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNATTAGCN_BC2_L5Ab_CIP_2.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNGCGCAGCN_BC2_L5Ac_CIP_3.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNAGAGCN_BC2_L5Ba_DYP_1.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNGTGAGCN_BC2_L5Bb_DYP_2.unique.hyb.p_values.tab"),
  file.path( hyb_p_val_dir,"Output_BC2", "Hyb_Annot", "all_NNNCACTAGCN_BC2_L5Bc_DYP_3.unique.hyb.p_values.tab") ) 

names(list_of_files ) <- c( "Hfq_1", 
                            "Hfq_2", 
                            "Hfq_3",
                            "CIP_1",
                            "CIP_2",
...
## Create a network, find connected components
```{r}
...
## get together the hybrids that were found in multiple experiments, overlapping in the same direction, or overlapping in different direction
...
## Find clusters where the distance covered by the two halfs are too long
```{r}
cluster_id_tbl_to_update <- cln_merged_positions_tbl %>%
  dplyr::mutate( left_len = abs(left_genomic_start - left_genomic_stop ) + 1 )  %>%
  dplyr::mutate( right_len = abs(right_genomic_start - right_genomic_stop) + 1 ) %>%
  dplyr::filter( left_len + right_len > cluster_max_distance ) %>%
  dplyr::select( cluster_id) %>%
  dplyr::inner_join( cluster_id_tbl, by = "cluster_id") %>%
  mutate( new_cluster_id = paste( cluster_id, row_number(), sep="_"  ) )

## Remove clusters that have regions that were too long
cluster_id_tbl_updated <- cluster_id_tbl %>%
  left_join( cluster_id_tbl_to_update, by=c("expt_hyb_id", "cluster_id")) %>%
  dplyr::filter( is.na( new_cluster_id)) %>%
...
merged_p_value <- filtered_tbl %>%
  inner_join(cluster_id_tbl_updated , by=c( "expt_hyb_id" = "expt_hyb_id")) %>%
...
non_merged_rows <- filtered_tbl %>%
  anti_join( cluster_id_tbl_updated, by=c("expt_hyb_id")) %>%
...
merged_rows_before_merge <- filtered_tbl %>%
  inner_join( cluster_id_tbl_updated, by=c("expt_hyb_id")) %>%
  mutate( combined_p_value  = p_value) %>%
...
```{r}
filtered_final_table %>%
  dplyr::filter( str_detect(left_hyb_id, "cluster" )) %>%
  dplyr::mutate( total_len = abs( left_genomic_start - left_genomic_stop ) + 1 + abs( right_genomic_start - right_genomic_stop ) + 1) %>%
  dplyr::filter( total_len >  cluster_max_distance)
```
